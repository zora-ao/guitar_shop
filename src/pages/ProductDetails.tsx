...
import { CreditCard, LifeBuoy, ShieldCheck, ShoppingCart, Store, Truck, ChevronDown, ChevronUp } from "lucide-react";
...
const ProductDetails = () => {
    ...
    const { data: reviews = [], refetch: refetchReviews } = useQuery({
        queryKey: ['reviews', id],
        queryFn: () => reviewApi.getReviews(Number(id)),
        enabled: !!id
    });
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFA] pb-20">
            <main className="max-w-7xl mx-auto px-6 md:px-12 py-10">
                        ...
                        <div className="mt-0 pt-10 border-t border-stone-100 grid grid-cols-2 gap-y-8 gap-x-4">
                            ...
                            <div className="flex gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-stone-500 mt-0.5">Pick-up at your store</p>
                                </div>
                            </div>
                            <div className="flex gap-3">
                                <div className="w-10 h-10 bg-stone-50 rounded-full flex items-center justify-center shrink-0">
                ...
                {/* --- NEW: REVIEWS SECTION (Original UI structure maintained) --- */}
                <section className="mt-24 border-t border-stone-200 pt-16">
                    <div className="flex items-center justify-between mb-12">
                        <div>
                            <h2 className="text-3xl font-serif text-stone-900">Reviews</h2>
                            <p className="text-sm text-stone-400 mt-1">Based on {reviews.length} verified purchases</p>
                        </div>
                        <button 
                            onClick={() => setShowReviews(!showReviews)}
                            className="flex items-center gap-2 px-6 py-2 bg-stone-100 hover:bg-stone-200 rounded-full text-xs font-bold tracking-widest transition-all"
                        >
                            {showReviews ? <><ChevronUp size={14}/> HIDE</> : <><ChevronDown size={14}/> SHOW</>}
                        </button>
                    </div>
                            ...
                            <div className="lg:col-span-1">
                                {hasUserReviewed ? (
                                    <div className="p-8 bg-stone-50 rounded-2xl border border-dashed border-stone-200 text-center">
                                        ...
                                        <h4 className="text-xs font-black uppercase tracking-widest text-stone-900 mb-2">Review Submitted</h4>
                                        <p className="text-[11px] text-stone-400 leading-relaxed">
                                            You've already shared your thoughts on this instrument. Thank you!
                                        </p>
                                    </div>
                                ) : (
                                    <ReviewForm productId={Number(id)} onReviewAdded={refetchReviews} />
                                )}
                            </div>
