...
const ProductList: React.FC = () => {
    ...
    const handleSaveEdit = (e: React.FormEvent) => {
        ...
        formData.append('is_best_seller', editingProduct.is_best_seller ? '1' : '0');

        // Filter: only keep URLs that exist on the server (exclude blobs and empties)
        const existingImages = previewsUrls.filter(url => 
            url && url.startsWith('http') && !url.startsWith('blob:')
        );
        formData.append('existing_images', JSON.stringify(existingImages));

        // Files: append actual File objects from the newFiles array
    ...
    useEffect(() => {
        if (editingProduct) {
            // Initialize with existing images up to 4 slots
    ...
    return (
        <div className="p-4 md:p-8 w-full max-w-6xl mx-auto">
            ...
            <div className="border border-stone-200 rounded-lg overflow-hidden bg-white shadow-sm">
                ...
                <div className="flex flex-col divide-y divide-stone-100">
                    {products.map((product) => (
                        <div key={product.id} className="flex flex-col md:grid md:grid-cols-[80px_1fr_120px_100px_100px_120px] items-start md:items-center p-4 md:py-3 md:px-4 hover:bg-stone-50 transition-colors gap-4 md:gap-0">
            ...
            {editingProduct && (
                <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-end md:items-center justify-center z-[100] p-0 md:p-4">
                        ...
                        <form onSubmit={handleSaveEdit} className="p-6 space-y-6 overflow-y-auto pb-32 md:pb-6">
                                            ...
                                            {previewsUrls[index] ? (
                                                <>
                                                    ...
                                                    <button type="button" onClick={() => {
                                                        if (previewsUrls[index].startsWith('blob:')) URL.revokeObjectURL(previewsUrls[index]);
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" className="w-full border-stone-200 border p-3 rounded-xl outline-none focus:ring-2 focus:ring-black" value={editingProduct.name} onChange={(e) => setEditingProduct({ ...editingProduct, name: e.target.value })} />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="flex items-center pt-4">
                                        <label className="flex items-center gap-3 cursor-pointer group">
                                            <input type="checkbox" checked={editingProduct.is_best_seller} onChange={(e) => setEditingProduct({ ...editingProduct, is_best_seller: e.target.checked })} className="w-5 h-5 accent-black rounded" />
                                            <span className="text-[10px] font-black uppercase text-stone-400 group-hover:text-black">Best Seller</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" className="w-full border-stone-200 border p-3 rounded-xl" value={editingProduct.price} onChange={(e) => setEditingProduct({ ...editingProduct, price: Number(e.target.value) })} />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] font-black uppercase text-stone-400 mb-1">Stock</label>
                                        <input type="number" className="w-full border-stone-200 border p-3 rounded-xl" value={editingProduct.stock} onChange={(e) => setEditingProduct({ ...editingProduct, stock: Number(e.target.value) })} />
                                    </div>
                                </div>
                            </div>

                            <button type="submit" disabled={updateMutation.isPending} className="w-full bg-black text-white py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-stone-800 disabled:bg-stone-400 transition-all shadow-lg">
