        ...
        <div className="max-w-7xl mx-auto px-4 py-10 font-sans">
            ...
            <div className="flex flex-col gap-4">
                {orders.map((order) => (
                    <div 
                        key={order.id} 
                        // Changed to grid-cols-12 for finer control
                        className="border border-gray-200 rounded-sm p-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-start text-sm text-gray-600"
                    >
                        
                        {/* Column 1: Much Longer (Spans 6/12) */}
                        <div className="flex gap-4 md:col-span-6">
                            <div className="bg-gray-100 p-3 rounded h-fit shrink-0">
                        ...
                        <div className="self-center md:col-span-2 flex justify-end">
                            <select
                                value={order.status}
                                onChange={(e) => handleStatusChange(order.id, e.target.value)}
                                // Added max-w-small to keep it compact
                                className="w-full max-w-[160px] border border-gray-300 p-2 rounded outline-none bg-white font-medium cursor-pointer"
                            >
